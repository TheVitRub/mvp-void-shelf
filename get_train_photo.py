...
# Функция для запуска active learning в отдельном потоке
def run_camera_active_learning(camera_ip: str, camera_id: int, shelf_coordinates: list):
    """
    Запускает active learning для одной камеры в отдельном потоке.
    ...
    try:
        ...
        print(f"[Камера {camera_id} ({camera_ip})] Запуск потока...")
        ...
        show_picture = ShowPicture(model=model, active_learning_dir=f"to_label_camera_{camera_id}")
...
for idx, camera_ip in enumerate(camera_ips, 1):
    thread = threading.Thread(
        target=run_camera_active_learning,
        args=(camera_ip, idx, shelf_coordinates),
        daemon=False,
